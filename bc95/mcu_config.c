...
* 说    明: 无
*************************************************************************/
void clear_bc95_buf(void)
{
	 memset(bc95_buf,0,BUF_MAX);
...
*						wait_time ----发送等待时间(单位：ms)
* 输    出: 无
* 返    回: 正确---1   错误---0
* 说    明: 无
*******************************************************************************/
u8 bc95_send_cmd(char *b,char *a,u8 times,u32 wait_time)         
{
	u8 i;
	i = 0;
	clear_bc95_buf();                    // 清除串口2缓存数据
	while(i < 5)                    
	{
    bc95_send_string(b);
		bc95_send_string("\r\n");	         // 回车换行
